...
var _ = Describe("PodManager", func() {
	...
	var podManagerConfig common.PodManagerConfig
	...
	BeforeEach(func() {
		...
		podManagerConfig = common.PodManagerConfig{
	...
	Describe("SchedulePodsRestart", func() {
		It("should only delete pods passed as arg", func() {
			...
			manager := common.NewPodManager(k8sInterface, common.NewNodeUpgradeStateProvider(k8sClient, log, eventRecorder), log, nil, eventRecorder)
	...
	Describe("ScheduleCheckOnPodCompletion", func() {
		It("should change the state of the node only after job completion", func() {
			...
			err := provider.ChangeNodeUpgradeState(ctx, node, common.UpgradeStateWaitForJobsRequired)
			Expect(err).To(Succeed())
			...
			manager := common.NewPodManager(k8sInterface, provider, log, nil, eventRecorder)
			...
			Expect(err).To(Succeed())
			Expect(node.Labels[common.GetUpgradeStateLabelKey()]).To(Equal(common.UpgradeStatePodDeletionRequired))
		...
		It("should not change the state of the node if workload pod is running", func() {
			...
			node, err = provider.GetNode(ctx, node.Name)
			Expect(err).To(Succeed())
			Expect(node.Labels[common.GetUpgradeStateLabelKey()]).To(Equal(common.UpgradeStateWaitForJobsRequired))
		...
		It("should change the state of the node if workload pod is running and timeout is reached", func() {
			...
			provider.ChangeNodeUpgradeAnnotation(ctx, node, common.GetWaitForPodCompletionStartTimeAnnotationKey(), startTime)
	...
	Describe("SchedulePodEviction", func() {
		...
		It("should delete all standalone gpu pods with force"+
			...
			// initialize upgrade state of the node
			provider := common.NewNodeUpgradeStateProvider(k8sClient, log, eventRecorder)
			err := provider.ChangeNodeUpgradeState(ctx, node, common.UpgradeStatePodDeletionRequired)
			Expect(err).To(Succeed())

			podManagerConfig.DeletionSpec.Force = true
			manager := common.NewPodManager(k8sInterface, provider, log, gpuPodSpecFilter, eventRecorder)
			...
			node, err = provider.GetNode(ctx, node.Name)
			Expect(err).To(Succeed())
			Expect(node.Labels[common.GetUpgradeStateLabelKey()]).To(Equal(common.UpgradeStatePodRestartRequired))
		})
		...
		It("should fail to delete all standalone gpu pods without force,"+
			...
			node, err = provider.GetNode(ctx, node.Name)
			Expect(err).To(Succeed())
			Expect(node.Labels[common.GetUpgradeStateLabelKey()]).To(Equal(common.UpgradeStateDrainRequired))
		})
